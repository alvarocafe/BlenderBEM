...
def plot_surface(number,tri,XYZ,surf):    
    numsurf=np.zeros(tri.shape[0])
    elsurf=[]
    for i in range(tri.shape[0]):
        if(surf[i] in number):
            numsurf[i]=10
            elsurf.append(i)
    show_results(tri,XYZ,numsurf)
    return elsurf

    
def plot_extremes(extremes,tri,XYZ,T):
    elem_color=np.zeros(tri.shape[0])
    for i in range(tri.shape[0]):
        if(T[i] > extremes[1]):
            elem_color[i]=10
        elif(T[i]<extremes[0]):
            elem_color[i]=-10           
    show_results(tri,XYZ,elem_color)
